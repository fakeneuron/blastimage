...
- **Type safety:** `strict` plus `noUncheckedIndexedAccess` (BI-036), so every array/record index read is `T | undefined`. Narrow it — a guard, `?.`, or a non-null assertion where the surrounding code already proves the index exists (the prevailing style in tests and in `lib/imagegenFs.ts` path walks).
