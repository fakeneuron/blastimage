...
3. **Link & load** — in blastimage, **🔗 Link imagegen** opens a folder picker (detected `imagegen/` folders on top, browse from there, or type an absolute path); every `rounds/rN/` auto-ingests, and **↻ Refresh rounds** re-lists and pulls in new ones. Works in any browser. The folder belongs to the **project**, not the app: the sidebar names it, switching project re-links its own folder, and a folder another project already owns is refused with an offer to switch to it. Linking a folder into a fresh, unnamed project names that project after the repo.
...
Sessions persist in `localStorage` — no accounts, no cloud project. Use **↓ Export** / **↑ Import** in the Project section to back up a whole workspace to a `.json` file and restore it (Import lands it as a fresh copy), so a project can move between browsers or devices.
