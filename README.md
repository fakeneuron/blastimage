...
3. **Link & load** — in blastimage, **🔗 Link imagegen** opens a folder picker (detected `imagegen/` folders on top, browse from there, or type an absolute path) then **↻ Load round** to ingest the batch into the review UI. Works in any browser, and the link is remembered.
