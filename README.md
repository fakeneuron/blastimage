...
3. **Link & load** — in blastimage, the sidebar **⋯** menu's **🔗 Link imagegen** opens a folder picker (detected `imagegen/` folders on top, browse from there, or type an absolute path); every `rounds/rN/` auto-ingests, and **↻ Refresh rounds** on the open rail re-lists and pulls in new ones. Works in any browser. The folder belongs to the **project**, not the app: the sidebar header names it, switching project re-links its own folder, and a folder another project already owns is refused with an offer to switch to it. Linking a folder into a fresh, unnamed project names that project after the repo.
...
1. **Create a project** — a default session opens on first launch; rename it from the header, or create a new one from the sidebar **⋯** menu.
2. **Add tasks** — each task has a base prompt and an optional reference photo selection (up to 3 active at once from the session library). To stage many at once, **🛠 Build** in the **⋯** menu composes a `tasks.json` from pasted prompts or `prompts/*.txt` files, and **⇪ Import** loads such a file into the session.
3. **Generate** — click **Generate** to produce a batch of candidate images via Grok Imagine. **⚡ Generate All** in the sidebar fires a batch for every eligible task at once and opens a stacked bulk-review view for one-pass review across tasks. In a plain browser the button is hidden and the header shows **Viewer**.
...
5. **Iterate** — undecided cards show **Iterate →** disabled until Keep (`Keep this image first`). Click **Iterate →** on a kept image to open the refine modal, edit the prompt, and generate a new round seeded by that image as the primary reference. Discarded and approved cards omit it (approved is final).
...
Sessions persist in `localStorage` — no accounts, no cloud project. Use **↓ Export** / **↑ Import** in the sidebar **⋯** menu to back up a whole workspace to a `.json` file and restore it (Import lands it as a fresh copy), so a project can move between browsers or devices.
