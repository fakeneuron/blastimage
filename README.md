...
3. **Link & load** — in blastimage, **🔗 Link imagegen** opens a folder picker (detected `imagegen/` folders on top, browse from there, or type an absolute path) then **↻ Load round** to ingest the batch into the review UI. Works in any browser. The folder belongs to the **project**, not the app: the sidebar names it, switching project re-links its own folder, and a folder another project already owns is refused with an offer to switch to it. Linking a folder into a fresh, unnamed project names that project after the repo.
