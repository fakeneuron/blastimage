...
4. **Review** — keep, discard, or approve each image. Clicking the active decision clears it; clearing an approve is fully reversible — the image is removed from `imagegen/approved/` and its `selection.json` entry is rewritten, so a mis-click needs no manual cleanup. Add star ratings and feedback notes. Toggle **Use as reference** to seed the keeper into the next round. Click any thumbnail (review grid or Gallery) to view it full-size; ← / → step through the set.
